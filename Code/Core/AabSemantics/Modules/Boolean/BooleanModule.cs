...
namespace AabSemantics.Modules.Boolean
{
	public class BooleanModule : ExtensionModule
	{
		...
		{
			Repositories.RegisterAttribute(IsValueAttribute.Value, language => language.GetExtension<LanguageBooleanModule>().Attributes.IsValue)
				.SerializeToXml(new Xml.IsValueAttribute())
				.SerializeToJson(new Xml.IsValueAttribute());
			Repositories.RegisterAttribute(IsBooleanAttribute.Value, language => language.GetExtension<LanguageBooleanModule>().Attributes.IsBoolean)
		...
		{
			Repositories.RegisterQuestion<CheckStatementQuestion>(language => language.GetExtension<LanguageBooleanModule>().Questions.Names.CheckStatementQuestion)
