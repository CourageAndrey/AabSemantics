...
{
	/// <summary>
	/// Renders text as a Makefile string.
	/// </summary>
	public class MakefileTextRender : IStructuredTextRender
	{
		...
		{
			var result = new StringBuilder(new String('#', text.Level));
			result.Append(' ');
		...
		{
			return renderDecorator(decorator, language, wrappingSymbol, wrappingSymbol);
		}
		...
		private StringBuilder renderDecorator(
			...
			String closeSymbol)
		{
			var result = new StringBuilder(openSymbol);
			result.Append(this.Render(decorator.InnerText, language));
			result.Append(closeSymbol);
			return result;
		}
